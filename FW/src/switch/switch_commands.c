...
{
  _switch_command_report_id[port] = report_id;
}

void set_ack(uint8_t port, uint8_t ack)
{
  _switch_command_buffer[port][12] = ack;
}

void set_command(uint8_t port, uint8_t command)
{
  _switch_command_buffer[port][13] = command;
}

void set_timer(uint8_t port)
{
  static int16_t _switch_timer = 0;
  _switch_command_buffer[port][0] = (uint8_t) _switch_timer;
  //printf("Td=%d \n", _switch_timer);
  _switch_timer+=3;
...
void set_battconn(uint8_t port)
{
  // Always set to USB connected
  _switch_command_buffer[port][1] = 0x01;
}

void set_devinfo(uint8_t port)
{
  ...
  _switch_command_buffer[port][14] = 0x03; // NS Firmware primary   (3.x)
  _switch_command_buffer[port][15] = 72; // NS Firmware secondary (x.72)
  ...
  // NES      - 0x09, 0x02
  // Genesis  - 0x0D, 0x02
  _switch_command_buffer[port][16] = 0x03; // Controller ID primary (Pro Controller)
  _switch_command_buffer[port][17] = 0x02; // Controller ID secondary

  /*_switch_command_buffer[18-23] = MAC ADDRESS;*/

  _switch_command_buffer[port][24] = 0x01;
  _switch_command_buffer[port][25] = 0x02; // It's 2 now? Ok.
}

void set_sub_triggertime(uint8_t port, uint16_t time_10_ms)
{
  ...
  {
      _switch_command_buffer[port][14 + i] = upper_ms;
      _switch_command_buffer[port][15 + i] = lower_ms;
  }
}
...
void info_set_mac(uint8_t port)
{
...
void info_set_init(uint8_t port)
{
  _switch_command_buffer[port][0] = 0x02;
}

void info_handler(uint8_t port, uint8_t info_code)
{
  clear_report(port);
  set_report_id(port, 0x81);
  ...
  switch(info_code)
  {
    ...
    default:
      ...
      _switch_command_buffer[port][0] = info_code;
      break;
  }

  tud_hid_n_report(port, _switch_command_report_id[port], _switch_command_buffer[port], 64);
}

void pairing_set(uint8_t port, uint8_t phase)
{
  ...
  switch(phase)
  {
    ...
    case 2:
      set_ack(port, 0x81);
      _switch_command_buffer[port][14] = 2;
      memcpy(&_switch_command_buffer[port][15], _switch_ltk, 16);
      break;
    case 3:
      set_ack(port, 0x81);
      _switch_command_buffer[port][14] = 3;
      break;
  }
}

// Handles a command, always 0x21 as a response ID
void command_handler(uint8_t port, uint8_t command, const uint8_t *data, uint16_t len)
{
  // Clear report
  clear_report(port);

  // Set report ID
  set_report_id(port, 0x21);
  ...
  switch(command)
  {
    ...
    case SW_CMD_GET_SPI:
      ...
      set_ack(port, 0x90);
      sw_spi_readfromaddress(port, data[12], data[11], data[15]);
      break;
  ...
  tud_hid_n_report(port, 0x21, _switch_command_buffer[port], 64);
}

// Handles an OUT report and responds accordingly.
void report_handler(uint8_t port, uint8_t report_id, uint8_t *data, uint16_t len)
{
  switch(report_id)
  {
    // We have command data and possibly rumble
    case SW_OUT_ID_RUMBLE_CMD:
      rumble_translate(port, &data[2]);
      command_handler(port, data[10], data, len);
      break;
...
void switch_commands_process(uint8_t port, sw_input_s *input_data)
{
  ...
  {
    if (_switch_reporting_mode[port] == 0x30)
    {
      clear_report(port);
      set_report_id(port, 0x30);
      set_timer(port);
      set_battconn(port);
      ...
      // Set input data
      _switch_command_buffer[port][2] = input_data->right_buttons;
      _switch_command_buffer[port][3] = input_data->shared_buttons;
      _switch_command_buffer[port][4] = input_data->left_buttons;
      ...
      _switch_command_buffer[port][10]  = (input_data->rs_y & 0xFF0) >> 4;
      _switch_command_buffer[port][11]  = _unknown_thing();
...
void switch_commands_future_handle(uint8_t port, uint8_t report_id, const uint8_t *data, uint16_t len)
{
...
{
  memcpy(&_switch_command_buffer[port][start_idx], data, len);
}
